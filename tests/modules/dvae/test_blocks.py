...
class TestDVAEBlocks(unittest.TestCase):
    ...
    def test_dim_Encoder_with_hidden_dim(self):
        x = torch.rand((1, 3, 128, 128))
        model = Encoder(in_channels=3, out_channels=20, hidden_channels=32, num_x2downsamples=2)
        expected_out_size = torch.Size([1, 20, 32, 32])
        model.eval()
        f = model.forward(x)
        self.assertEqual(expected_out_size, f.sizes())
    ...
    def test_dim_Decoder_with_hidden_dim(self):
        ...
        model = Decoder(in_channels=32, out_channels=3, hidden_channels=64, num_x2upsamples=2)
