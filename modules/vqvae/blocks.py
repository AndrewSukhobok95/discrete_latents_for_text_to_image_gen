...
class Encoder(nn.Module):
    def __init__(self,
        ...
        for _ in range(num_downsamples):
            ...
            down_sample_block += [
                ...
                ResidualStack(in_channels=_out_ch, out_channels=_out_ch,
                              num_residual_layers=num_resid_downsample,
                              bias=bias, use_bn=use_bn)
            ]
        ...
        self._res_block = ResidualStack(in_channels=out_channels, out_channels=out_channels,
                                        num_residual_layers=num_resid_bottleneck,
                                        bias=bias, use_bn=use_bn)
        self._conv1x1 = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.use_conv1x1 = use_conv1x1
        self._conv1x1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=bias)
        _out_ch = in_channels
        ...
        for _ in range(num_upsamples):
            ...
            up_sample_block += [
                UpSampleX2(in_channels=_in_ch, out_channels=_out_ch,
                           bias=bias, use_bn=use_bn),
                ResidualStack(in_channels=_out_ch, out_channels=_out_ch,
                              num_residual_layers=num_resid_upsample,
                              bias=bias, use_bn=use_bn)
            ]
