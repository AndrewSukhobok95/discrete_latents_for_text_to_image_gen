...
from modules.tadvae_discriminator.blocks import DownSampleX2
...
class Discriminator(nn.Module):
    def __init__(self, txt_in_dim, hidden_dim=512):
        ...
        self.img_vectorizer_list = nn.ModuleList([
            nn.Sequential(
    ...
    def forward(self, img, txt, txt_mask, only_conditional=False):
        ...
        img_feats = [img_feat_1, img_feat_2, img_feat_3]
        ...
        for i in range(len(img_feats)):
            img_feat = self.img_vectorizer_list[i](img_feats[i]).squeeze(-1)
            W_cond = self.txt_weight[i](txt)
